  ...
  Widget _buildCoursesList(List<dynamic>? courses) {
    ...
    return CustomContainer(
      ...
      child: ListView.separated(
        ...
        shrinkWrap: true,
        itemCount: courses.length > 2 ? 2 : courses.length,
        // يعرض أول كورسين بس في البروفايل
        separatorBuilder: (context, index) =>
            Divider(color: AppColors.c737373.withOpacity(0.5)),
        itemBuilder: (context, index) {
          ...
          return RunningCourseItemView(
            ...
            progress:
                (double.tryParse(
                      course.progressPercentage?.toString() ?? '0',
                    ) ??
                    0.0) /
                100,
            courseId: course.id,
    ...
    return Column(
      children: [
        const Align(
          ...
          child: Text(
            'ملخص نشاط الطالب',
            style: TextStyle(fontSize: 18.66, fontWeight: FontWeight.w700),
          ),
        ),
        const SizedBox(height: 10),
        CustomContainer(
